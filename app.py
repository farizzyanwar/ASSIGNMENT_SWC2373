...
def index():
    if request.method == 'POST':
        ...
        if 'errors' in user_info:
            ...
            return render_template('user_info.html', user_info=user_info, rooms=rooms_data['items'], access_token=access_token)
...
@app.route('/add_room/<access_token>', methods=['POST'])
def add_room(access_token):
    headers = {
        'Authorization': f'Bearer {access_token}'
    }

    # Get the room title from the form
    ...
    create_room_response = requests.post(f'{WEBEX_API_URL}/rooms', headers=headers, json=room_data)
    
    if create_room_response.status_code == 200:
        # Redirect to the user info page to show the updated room list
        return redirect(url_for('index'))
    else:
        return "Failed to create the room. Please try again."
